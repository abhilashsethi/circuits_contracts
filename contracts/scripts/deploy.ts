...
async function main() {
  ...
  // dlpc digital registry
  const DLPCAssetRegistry__factory = await ethers.getContractFactory("DLPCAssetRegistry");
  const DLPCAssetRegistry = await DLPCAssetRegistry__factory.deploy(uri);
  await DLPCAssetRegistry.deployed();
  console.log(`DLPCAssetRegistry deployed at: ${DLPCAssetRegistry.address}`);

  const DlpcMetaTx__factory = await ethers.getContractFactory("MetaTx");
  const DlpcMetaTx = await DlpcMetaTx__factory.deploy();
  await DlpcMetaTx.deployed();
  console.log(`DlpcMetaTx deployed at: ${DlpcMetaTx.address}`);
  ...
  await DLPCAssetRegistry.setMetaTxContractAddress(DlpcMetaTx.address);
  await sleep(5000);
  console.log(`Dlpc meta tx address set`);
  await DlpcMetaTx.setVerifier(verifier.address);
  await sleep(5000);
  console.log(`verifier set in DLPC Meta Tx`)
  await DlpcMetaTx.setDigitalAssetRegistry(DLPCAssetRegistry.address);
  console.log(`DLPC DigitalAssetRegistry set in Meta Tx`)
